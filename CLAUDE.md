...
| `L2_MAX_SENDER_WORKERS` | (unset) | Sender goroutine pool, and **the single most important knob on a high-latency path -- LOWER is usually better, which is the opposite of what the default suggests**. Each worker is pinned to ONE account and gated on that account's batch ack, so in-flight work is `workers x 20` and ack latency is that divided by the throughput the path can serve. At the 4000 default into a path serving ~3,000 tx/s that is 80,000 transactions in flight and ~27 s of ack latency against a 30 s `L2_BATCH_ACK_TIMEOUT`, which collapses rather than slows: measured on Tickr (PRST-4459) 53.6% of submissions refused and two zero-transaction stretches of 56 s and 9 s. Dropping to 300 on the same rig gave **2,913.8 tx/s against 2,054.8, zero failures out of 528,392 sends, and no outages**. Because a blocking `eth_sendRawTransaction` plus serial batch handling means each account lands one transaction per block, throughput is `active accounts x block rate` -- so size this as `target_tx_per_s / block_rate`, not as high as the funded account count allows. Raising it lifts the per-block ceiling but not the average, and starts drawing refusals at the edge. Sender concurrency still scales in step (invariant: 4x the pool). |
