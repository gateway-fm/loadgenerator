...
func TestERC20RecipientPool_EnvParsing(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run("ERC20_RECIPIENT_POOL="+tc.val, func(t *testing.T) {
			t.Setenv(EnvERC20RecipientPool, tc.val)
			if got := erc20PoolSizeFromEnv(); got != tc.want {
				t.Errorf("erc20PoolSizeFromEnv() = %d, want %d", got, tc.want)
			}
		})
	}
}

func TestERC20RecipientPool_SeedOverride(t *testing.T) {
	t.Setenv(EnvERC20RecipientPoolSeed, "")
	if got := erc20PoolSeedFromEnv(); got != defaultERC20RecipientPoolSeed {
		t.Errorf("default seed = %q, want %q", got, defaultERC20RecipientPoolSeed)
	}
	t.Setenv(EnvERC20RecipientPoolSeed, "custom")
	if got := erc20PoolSeedFromEnv(); got != "custom" {
		t.Errorf("overridden seed = %q, want %q", got, "custom")
	}
}
