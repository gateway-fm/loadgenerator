...
| `L2_CLIENT_MAX_RETRIES` | (unset) | Number of ATTEMPTS on those clients (not retries): `1` is a single attempt with no retry, `3` is three attempts. Unset or `0` keeps the client default of 3 retries (4 attempts). Retries are re-counted by a per-key rate limiter that meters batch items. |
...
| `L2_FORCE_HTTP2` | `false` | Enable HTTP/2 on the builder/L2 transport. Over HTTP/1.1 each concurrent request needs its own connection, so N in flight costs N TLS handshakes and queues behind `MaxConnsPerHost`; HTTP/2 multiplexes them as streams over one connection. Only matters against a TLS endpoint, and only helps because it removes latency rather than adding parallelism. Falls back silently to HTTP/1.1 if the server does not negotiate h2 — verify the protocol before crediting a result to it. |
| `L2_PIPELINE_BATCH_DEPTH` | `1` | Batches in flight **per sender account**. 1 is strict serialisation (historical default): the worker waits for every ack before building the next batch, so consecutive nonces for one account can never race — correct on a chain with no mempool, where an early nonce is refused rather than queued. Above 1 relies on nitro's nonce-failure cache parking a too-high nonce and retrying it; **watch `arb_sequencer_noncefailurecache_overflow`**, non-zero means a nonce was dropped and the arm is untrustworthy. Unlike more workers or connections, depth does not inflate the round trip — it overlaps waits that were serial. |
...
| `ERC20_RECIPIENT_POOL` | (unset) | Bounds the `erc20-transfer` recipient set to N addresses derived as `sha256(seed \|\| be64(i))[:20]`. Unset keeps the historical behaviour: a fresh random recipient per transfer, so holders grow 1:1 with transfers — 10M transfers makes ~10M holders and every downstream indexer balance write is an INSERT. Setting N makes the holder:transfer ratio a knob, which is the shape an indexer actually sees. **A malformed value is fatal at startup, not a fallback to random** — `0`, a negative, or the Go-literal form `1_000_000` exits with an error, because silently reverting to the unbounded workload would report all-random gas and tx/s under a pool label. Addresses are derived, never stored, so a 1M pool costs no memory. |
| | | **The warm-SSTORE saving depends on transfers-per-holder, not on setting the flag.** Selection is uniform, so the warm fraction is `1 − (N/T)(1 − e^(−T/N))` for T transfers: ~90% at T/N = 10, ~37% at T/N = 1, ~21% at T/N = 0.5. `ERC20_RECIPIENT_POOL=1000000` over 10M transfers is ~10 per holder and mostly warm (~30k gas); the same N over a 500k-transfer arm touches ~393k distinct addresses, leaves ~79% of transfers first-touch cold (~52k), and profiles close to an all-random run. Size N against the transfer count of the arm, not once. |
| | | **Gas and throughput from a pooled run are not comparable with an all-random one**, and a re-run is not comparable with a first run: the ERC-20 address is cached per chain ID and the seed is fixed, so a second arm on the same chain reuses the same holder set with its balance slots already non-zero and starts fully warm. Quote the mode, N, the transfer count and whether the chain was fresh alongside any gas figure. |
| `ERC20_RECIPIENT_POOL_SEED` | `gasstorm-erc20-recipient-pool-v1` | Seed the recipient pool is derived from. Fixed by default so `(seed, N)` alone reproduces the same holder set. That makes the run *definition* replayable — it does not make results replayable, since the same holder set on an already-warmed chain costs less gas than on a fresh one. Change it to get a disjoint holder set on the same chain. |
